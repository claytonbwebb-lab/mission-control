...
export async function registerRoutes(
  ...
  app.get("/api/ai/usage", async (req, res) => {
    ...
    if (!apiKey) {
      return res.json({
        remainingCredit: 142.58,
        usageThisMonth: 37.42,
        usageLastMonth: 89.15,
        currency: "USD",
      });
    }
  ...
  app.get("/api/ai/usage/daily", async (req, res) => {
    ...
    if (!apiKey) {
      ...
      for (let i = 6; i >= 0; i--) {
        ...
        result.push({
          ...
          outputTokens: Math.floor(Math.random() * 150000) + 30000,
        });
      }
    ...
    try {
      ...
      return res.json(entries.map((entry: Record<string, unknown>) => ({
  ...
  app.get("/api/ai/usage/recent", async (req, res) => {
    const apiKey = process.env.ANTHROPIC_API_KEY;
    if (!apiKey) {
      const models = ["claude-sonnet-4-20250514", "claude-3.5-haiku-20241022", "claude-sonnet-4-20250514"];
      const result = [];
      for (let i = 0; i < 20; i++) {
        const d = new Date();
        d.setHours(d.getHours() - i * 3);
        const input = Math.floor(Math.random() * 8000) + 500;
        const output = Math.floor(Math.random() * 4000) + 200;
        result.push({
          id: randomUUID(),
          date: d.toISOString(),
          ...
          inputTokens: input,
          outputTokens: output,
          estimatedCost: parseFloat(((input * 0.003 + output * 0.015) / 1000).toFixed(4)),
        });
      }
      return res.json(result);
    }
    ...
    try {
      const response = await fetch("https://api.anthropic.com/v1/usage/messages?limit=20", {
        headers: { "x-api-key": apiKey, "anthropic-version": "2023-06-01", "content-type": "application/json" },
      });
      if (!response.ok) return res.status(response.status).json({ error: await response.text() });
      const data = await response.json();
